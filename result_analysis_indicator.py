...
def plot_size_vs_verification_indicator(result):
    # 绘制散点图
...
logger.debug(f"\n{result}")
# intra question processing
if config["postProcessMethod"]["intraQuestion"].lower() == "sum":
    result = result.map(sum)
elif config["postProcessMethod"]["intraQuestion"].lower() == "mean":
    result = result.map(np.mean)
else:
    logger.critical(f"Invalid intraQuestion post processing method: {config['postProcessMethod']['intraQuestion']}")
    raise ValueError(f"Invalid intraQuestion post processing method: {config['postProcessMethod']['intraQuestion']}")

# inter question processing
if config["postProcessMethod"]["interQuestion"].lower() == "sum":
    result = result.apply(sum, axis=1)
elif config["postProcessMethod"]["interQuestion"].lower() == "mean":
    result = result.apply(np.mean, axis=1)
else:
    logger.critical(f"Invalid interQuestion post processing method: {config['postProcessMethod']['interQuestion']}")
    raise ValueError(f"Invalid interQuestion post processing method: {config['postProcessMethod']['interQuestion']}")
...
plot_size_vs_verification_indicator(result)
